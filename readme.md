...
## История и разработка проекта: кратко
...
![First Version](https://drive.google.com/uc?export=view&id=13KU6TeqMen2jOGXHUuKaNrvEkB4uROCy "First Version")
![First Version](https://drive.google.com/uc?export=view&id=1R4iwU7iHK6ePMx1kEPtTranqww2S192Y "First Version")
#### скриншоты первой версии

![Middle Version](https://drive.google.com/uc?export=view&id=1DWetSMEXBpbM1R8mB-BI9woDq91TFgVr "Middle Version")
![Middle Version](https://drive.google.com/uc?export=view&id=1elzpyogvuO1NBWC50u9tClAkRQdNMYgA "Middle Version")
#### скриншоты промежуточной версии

Спустя почти полгода после первой сборки я посмотрел на свой проект более опытным и свежим взглядом и решил его переработать, используя новый для себя рендер **URP** (вместо **LWRP**). **Universal Render Pipeline** требовал оптимизации в части 3D-моделей и реализации механик: я практически заново построил все 3D-модели, сделав их модульными вместо монолитных, сделал новые материалы и шейдеры к ним средствами **ShaderGraph**’а, а также переписал большую часть кода, внедряя новые паттерны и методы. Поэтому можно сказать, что сейчас игра существует и разрабатывается в своей второй версии.
...
## Как и почему я решил разрабатывать игры
...
Я увлекался видеоиграми с самого детсва, моей первой , и через пару лет после появления у меня дома первого ПК сделал первые шаги в разработке их разработка меня заинтересовала в момент появления у меня дома ПК: так я начал делать карты для **Counter-Strike 1.6**, писать миссии для **Grand Theft Auto: San Andreas** и разрабатывать полноценные модификации для **Far Cry**.

Полноценно заняться изучением цикла разработки игры на игровом движке у меня вышло только в 2017-ом году, когда у меня появился более-менее подходящий ноутбук и закончилась учёба в университете: сначала **Unreal Engine 4** из-за обилия видеоуроков в **YouTube**, а потом **Unity 3D** в виду того, что на моём рабочем месте я смог выдвинуть предложение о том, чтобы создать полноценную 3D-визуализацию на базе этого игрового движка.

Реализовав проект я стал по-немногу за счет экспериментов узнавать те или иные составляющие движка: сначала азы языка **C#**, потом различные базовые механики игр. Очень сильно увлёкся изучением сетевого взаимодействия, так как стал изучать возможности фреймворка **.NET Core**, после чего родилась идея сделать мультиплеерную игру.

В начале 2020 года у меня появилось стойкое желание реализовать свой игровой проект, поэтому оставалось только найти подходящую идею.

## The Sanctuary 3D: идея и реализация игры

Во время объявленной в России самоизоляции весной 2020 года я купил подписку на **Spotify**, в котором мои рекомендации плавно заполнились западным хип-хопом. Одним из вечеров я задался вопросом об истории становления **Дрейка** поп-артистом №1, так как в те дни о нем говорило много международных СМИ по причине релиза его нового альбома. Из новостных статей я узнал, что у Дрейка есть роскошный особняк в Торонто, на территории которого построен личный баскетбольный зал "The Sanctuary". Я нашел несколько фото и видео зала в медиа, дизайн интерьра мне показался интересным и тогда я подумал: "А что, если создать 3D-копию этого зала в VR, добавить игровые механики, а на фоне с эффектом реверберации (эхо) в зале крутить мои треки"? То есть изначально проект задумывался как площадка привлечения внимания к моей музыке, но позже я решил разработать кроссплатформенную игру и добавить туда треки начинающих музыкантов для взаимного продвижения.

### Основной сеттинг

В качестве основы игры служит баскетбольная площадка, воссозданная мной с реально существующего зала Дрейка "The Sanctuary" средствами **Blender**, мой опыт работы в котором на тот момент был уже более 2-х лет.
...
В один момент я осознал, что просто бросать мяч в кольцо будет интересно только фанатам Дрейка, а остальным, кто скачает приложение из игрового магазина по случайной рекомендации, захотется расширенного геймплея. По этой причине я разработал несколько режимов игры и добавил сетеове взаимодействие пользователей. 
...
В дальнейшем же хотелось бы внедрить мастер-сервер и сеть серверов в разных точках, чтобы клиенты разных стран не испытывали проблем из-за высокого пинга.
Всё это реализовано при помощи ASP.NET Core, SignalR и MongoDB. 

## Заключение

Сейчас все усилия направлены на завершение разработки общего каркаса, который в дальнейшем будет разделён по особенностям игры на оставшихся платформах.

Каждый день я работаю над данным игровым проктом, благодаря чему всегда узнаю что-то новое для себя в игровом движке, языке программирования и 3D-моделировании, что, в свою очередь, ещё сильнее вызывает у меня интерес к геймдеву.